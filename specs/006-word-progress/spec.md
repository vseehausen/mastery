...
**Independent Test**: Can be fully tested by completing a learning session with words at different stages and verifying that micro-feedback appears at the moment of stage transitions (e.g., "Stabilizing", "Known now"). Delivers immediate motivational value to users.
...
2. **Given** I'm reviewing a word for the first time using a definition cue (non-translation), **When** I answer correctly, **Then** I see brief micro-feedback "Known now" displayed
...
**Independent Test**: Can be tested by completing a session where multiple words transition stages, then verifying the recap shows accurate counts (e.g., "2 words stabilized • 1 word became Known"). Delivers session-level accomplishment signal.

**Acceptance Scenarios**:

1. **Given** I just finished a session where 2 words stabilized and 1 became Known, **When** the session completes, **Then** I see a compact recap: "2 words stabilized • 1 word became Known"
...
4. **Given** I finished a session with multiple transition types, **When** I see the recap, **Then** transitions are summarized by type in order of significance (Mastered → Known → Stabilizing)
...
**Independent Test**: Can be tested by viewing the vocabulary list and verifying that each word displays its current stage (New, Practicing, Stabilizing, Known, Mastered). Delivers portfolio-level competence awareness.

**Acceptance Scenarios**:

1. **Given** I'm viewing my vocabulary list, **When** I see a word I just captured from reading but haven't reviewed yet, **Then** its stage is shown as "New"
2. **Given** I'm viewing my vocabulary list, **When** I see a word I've been practicing in SRS, **Then** its stage is shown as "Practicing", "Stabilizing", "Known", or "Mastered" based on its learning history
...
- What happens when a word regresses (user forgets it after multiple lapses)? The stage should reflect current competence, potentially moving back from Known → Practicing if retrieval success drops
- How does the system handle words captured but never reviewed? They remain at "New" indefinitely until first review
- What happens when a user completes a session with only translation cues (no non-translation retrieval)? Words can progress to Stabilizing but not to Known
- How are simultaneous transitions handled (e.g., a word becomes both Stabilizing and Known in same session)? Show the highest/most significant transition only
- What happens if enrichment fails or is incomplete? Word remains at "New" (enrichment is informational only, doesn't block progression)

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST track word progress through defined stages: New → Practicing → Stabilizing → Known → Mastered
- **FR-002**: System MUST update word stage based on logged learning events (captures, recalls, retrieval types, stability metrics)
- **FR-003**: System MUST display brief micro-feedback ("Stabilizing", "Known now") when a word transitions to a new stage during a session
- **FR-004**: System MUST generate a post-session recap summarizing progress transitions (e.g., "2 words stabilized • 1 word became Known")
- **FR-005**: System MUST only promote a word to "Known" after at least one successful non-translation retrieval (definition, synonym, or context cue)
...
- **Word Progress Stage**: Represents a word's current competence level (New, Practicing, Stabilizing, Known, Mastered). Derived from user-driven learning events and current SRS metrics.
...
- **Stage Transition**: Represents a change from one progress stage to another, triggered by specific learning event patterns (e.g., first non-translation success → Known).
...
- **Cue Type**: Classification of how a word was presented during recall (translation, definition, synonym, cloze, disambiguation). Used to determine eligibility for "Known" status.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: At least 60% of users see one or more words reach "Known" status within their first 3 weeks of regular use
