...
- Q: How are disambiguation and cloze prompt outcomes mapped to FSRS ratings? → A: Disambiguation and cloze cues use objective measurement instead of self-rating buttons. The system records response time and correctness, then auto-derives an FSRS rating: correct + fast (<5s) → Easy, correct + normal (5-15s) → Good, correct + slow (>15s) → Hard, incorrect → Again. Definition and synonym cues keep standard self-rating buttons (Again/Hard/Good/Easy) since they are open recall with no objectively verifiable answer.
