-- Migration: Update get_session_cards RPC to use global_dictionary
-- Feature: 008-global-dictionary
-- Date: 2026-02-10
--
-- Rewrites get_session_cards to:
-- - JOIN vocabulary → global_dictionary instead of aggregating meanings + cues
-- - Include all global dict enrichment fields
-- - Include user overrides from vocabulary.overrides
-- - Add encounter_context subquery (most recent encounter with non-empty context)
-- - Include eligibility flags: has_confusables, non_translation_success_count
-- - Filter WHERE v.global_dictionary_id IS NOT NULL (only enriched words)
