-- Migration: Deprioritize cards reviewed today in get_session_cards
-- Date: 2026-02-11
--
-- Problem: Quick reviews show the same words from earlier sessions because
-- the RPC doesn't deprioritize recently-reviewed cards.
--
-- Solution: Add ordering to push today's already-reviewed cards to the end
-- without hard exclusion. They're still available if user exhausts all other cards.
...
AS $$
  ...
  ORDER BY
    -- Cards reviewed today get deprioritized
...
Cards reviewed today are deprioritized but still available if needed.';
