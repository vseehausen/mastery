-- Migration: UNION-based get_session_cards for separate review/new word limits
-- Date: 2026-02-12
--
-- Replaces the single-limit get_session_cards with a UNION ALL approach:
...
-- - Separate limits for reviews and new words
-- - Exclude IDs prevent duplicates across batches
-- Uses global dictionary lemma as stem (COALESCE: gd.word > v.stem > v.word)
-- All varchar columns explicitly cast to text for type consistency.
...
Uses global_dictionary.word (lemma) as stem. Only returns enriched vocabulary.';
