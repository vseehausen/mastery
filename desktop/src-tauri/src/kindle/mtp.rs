...
impl MtpDevice {
    pub fn find_kindle() -> Result<Self, String> {
        for device in rusb::devices().map_err(|e| format!("USB error: {}", e))?.iter() {
    ...
    fn open_device(device: Device<GlobalContext>) -> Result<Self, String> {
                ...
                if desc.class_code() == 6 || desc.class_code() == 0xff {
                    ...
                    for endpoint in desc.endpoint_descriptors() {
                        match (endpoint.direction(), endpoint.transfer_type()) {
                            (rusb::Direction::In, rusb::TransferType::Bulk) => ep_in = endpoint.address(),
                            (rusb::Direction::Out, rusb::TransferType::Bulk) => ep_out = endpoint.address(),
                            _ => {}
                        }
                    }
        ...
        Ok(Self { handle, ep_in, ep_out, transaction_id: 0 })
    }
    ...
    fn build_command(&mut self, code: u16, params: &[u32]) -> Vec<u8> {
        ...
        buf.write_u16::<LittleEndian>(PTP_CONTAINER_COMMAND).unwrap();
    ...
    fn send_command(&mut self, code: u16, params: &[u32], timeout: Duration) -> Result<(), String> {
        ...
        self.handle.write_bulk(self.ep_out, &cmd, timeout)
    ...
    fn read_response(&mut self, timeout: Duration) -> Result<(u16, Vec<u32>), String> {
        ...
        let n = self.handle.read_bulk(self.ep_in, &mut buf, timeout)
            .map_err(|e| format!("Read failed: {}", e))?;
        
        if n < 12 { return Err(format!("Response too short: {} bytes", n)); }
    ...
    fn read_data(&mut self) -> Result<Vec<u8>, String> {
        ...
        loop {
            ...
            if first && n >= 12 {
                ...
                if container_type == PTP_CONTAINER_RESPONSE { return Ok(buf[..n].to_vec()); }
            ...
            if all_data.len() + 12 >= expected_len as usize { break; }
        }
        Ok(all_data)
    }
    ...
    fn get_storage_ids(&mut self) -> Result<Vec<u32>, String> {
        ...
        if code != PTP_RC_OK { return Err(format!("GetStorageIDs failed: {:#06x}", code)); }
        if data.len() < 4 { return Err("No storage data".to_string()); }
    ...
    fn get_object_handles(&mut self, storage_id: u32, parent: u32) -> Result<Vec<u32>, String> {
        ...
        if code != PTP_RC_OK { return Err(format!("GetObjectHandles failed: {:#06x}", code)); }
    ...
    fn get_object_info(&mut self, handle: u32) -> Result<ObjectInfo, String> {
        ...
        if code != PTP_RC_OK { return Err(format!("GetObjectInfo failed: {:#06x}", code)); }
        ...
        let filename = self.read_ptp_string(&data, 52).unwrap_or_else(|_| "?".to_string());
        
        Ok(ObjectInfo { format, filename, size })
    }

    fn read_ptp_string(&self, data: &[u8], offset: usize) -> Result<String, String> {
        if offset >= data.len() { return Ok(String::new()); }
    ...
    fn get_object(&mut self, handle: u32) -> Result<Vec<u8>, String> {
        ...
        loop {
            let n = self.handle.read_bulk(self.ep_in, &mut buf, Duration::from_secs(30))
                .map_err(|e| format!("Read failed: {}", e))?;
            ...
            if first && n >= 12 {
                ...
                expected_len = cursor.read_u32::<LittleEndian>().unwrap();
                let container_type = cursor.read_u16::<LittleEndian>().unwrap();
                if container_type == PTP_CONTAINER_RESPONSE { break; }
                all_data.extend_from_slice(&buf[12..n]);
                first = false;
            } else {
                all_data.extend_from_slice(&buf[..n]);
        ...
        let (code, _) = self.read_response(TIMEOUT)?;
        if code != PTP_RC_OK { return Err(format!("GetObject failed: {:#06x}", code)); }
        
        Ok(all_data)
    }

    fn find_folder(&mut self, storage_id: u32, parent: u32, name: &str) -> Result<Option<u32>, String> {
    ...
    pub fn download_vocab_db(&mut self, output_path: &Path) -> Result<u64, String> {
        ...
        for storage_id in storage_ids {
            ...
            if let Some(system_handle) = self.find_folder(storage_id, 0xFFFFFFFF, "system")? {
                ...
                if let Some(vocab_handle) = self.find_folder(storage_id, system_handle, "vocabulary")? {
                    // Find vocab.db
                    let handles = self.get_object_handles(storage_id, vocab_handle)?;
                    for handle in handles {
                        let info = self.get_object_info(handle)?;
                        if info.filename.to_lowercase() == "vocab.db" {
                            let data = self.get_object(handle)?;
